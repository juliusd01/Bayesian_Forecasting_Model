...
cmax_1 = 7
cmax_2 = 60    
cmax <- cmax_2    # set cmax <- cmax_2 if you want to switch to model with 60 choices per unit
lgtdata_sim=NULL
hdata=NULL
for (i in 1:nunits) {
...
Prior = list(ncomp=1)
out_UC = rhierMnlRwMixture(Data=E_Data,Mcmc=Mcmc,Prior=Prior)

# posterior draws of mu -> population level
bbarmc=matrix(double(6000*3),ncol=3)

for (r in 1:6000){
  bbarmc[r,] = out_UC$nmix$compdraw[[r]][[1]]$mu
}

betadrawconverged = out_UC$betadraw[,,3001:6000]
...
windows()
plot(out_UC$loglike, type = 'l')

# Density plots for price, brand A, and brand B coefficients
windows()
par(mfrow = c(2, 2))  # 2x2 layout for plots
plot(density(betaexchange[, 1]), main = "Price Coefficient (??1)", col = "blue", lwd = 2); grid()
plot(density(betaexchange[, 2]), main = "Brand A Coefficient (??2)", col = "red", lwd = 2); grid()
plot(density(betaexchange[, 3]), main = "Brand B Coefficient (??3)", col = "green", lwd = 2); grid()
smoothScatter(betaexchange[, 2], betaexchange[, 3],
              main = "Brand A vs Brand B (??2 vs ??3)", xlab = "??2 (Brand A)", ylab = "??3 (Brand B)"); 
abline(0, 1, col = "black", lwd = 2)  # Line where ??2 = ??3
...
rownames(beta_summary) <- c("Price (??1)", "Brand A (??2)", "Brand B (??3)")
beta_summary
...
legend("center", legend = c("Price (??1)", "Brand A (??2)", "Brand B (??3)"),
       col = 1:3, lty = 1, lwd = 2)
...
colnames(summary_stats_df) <- c("??1 (Price)", "??2 (Brand A)", "??3 (Brand B)")
