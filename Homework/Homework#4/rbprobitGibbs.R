...
# Task 5: Recover Data-Generating Parameters
# Simulate data
simulate_binary_probit_data <- function(n, p, beta) {
  X <- matrix(rnorm(n * p), nrow = n, ncol = p)  # Predictors
  epsilon <- rnorm(n)  # Error terms
  z <- X %*% beta + epsilon  # Latent variables
  y <- ifelse(z > 0, 1, 0)  # Binary outcomes
  list(X = X, y = y, z = z)
}
...
true_beta <- runif(p, -1, 1)  # True beta coefficients

# Generate binary probit data
data <- simulate_binary_probit_data(n, p, true_beta)

# Run Gibbs sampler
R <- 1000  # Number of iterations
Prior <- list(betabar = rep(0, p), A = 0.01 * diag(p))
Mcmc <- list(R = R, keep = 10)

results <- rbprobitGibbs(Data = data, Prior = Prior, Mcmc = Mcmc)

# Compare posterior mean of beta with true beta
posterior_mean_beta <- colMeans(results$betadraw)
cat("True beta:", true_beta, "\n")
cat("Posterior mean beta:", posterior_mean_beta, "\n")
